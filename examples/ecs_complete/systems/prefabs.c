...
void CreatePrefabs(ecs_world_t *world) {
    ...
    ecs_add(world, function_prefab, EcsPrefab);
    ...
    ecs_set(world, function_prefab, Transform, {.needs_update = true});
    ...
    ecs_set(world, file_prefab, Transform, {.needs_update = true});
    ...
    ecs_set(world, header_slot, Transform, {.needs_update = true});
    ...
    ecs_set(world, func_slot, Transform, {.needs_update = true});
...
ecs_entity_t CreateFunctionInstance(ecs_world_t *world, const char* name, Vector3 position) {
    ...
    ecs_set(world, instance, Position, position);
    ...
    ecs_set(world, instance, TextContent, text_content);
    
    ecs_set(world, instance, Transform, {.needs_update = true});
...
ecs_entity_t CreateFileInstance(ecs_world_t *world, const char* filename) {
    ...
    ecs_set(world, file_instance, TextContent, text_content);
    
    ecs_set(world, file_instance, Transform, {.needs_update = true});
    ...
    while (ecs_children_next(&slot_it)) {
        for (int i = 0; i < slot_it.count; i++) {
            ...
            if (slot_name && strcmp(slot_name, "HeaderSection") == 0) {
                ...
                ecs_set(world, slot_it.entities[i], TextContent, header_text);
            }
        }
    }
...
void CreateCodeHierarchy(ecs_world_t *world) {
    ...
    ecs_set(world, scene_root, Transform, {.needs_update = true});
    ...
    ecs_set(world, code_file, Transform, {.needs_update = true});
    ...
    ecs_set(world, main_function, Transform, {.needs_update = true});
    ...
    ecs_set(world, if_block, Transform, {.needs_update = true});
...
void SetupCodeDependencies(ecs_world_t *world) {
    ...
    ecs_set(world, header_file, Transform, {.needs_update = true});
    ...
    ecs_set(world, source_file, Transform, {.needs_update = true});
    ...
    ecs_set(world, main_func, Transform, {.needs_update = true});
    ...
    ecs_set(world, printf_func, Transform, {.needs_update = true});
